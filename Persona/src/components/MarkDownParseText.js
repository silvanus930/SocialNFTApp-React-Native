...
import { View, Linking, Text, StyleSheet, Animated, Dimensions, Platform } from 'react-native';
            ...
            {[...textSnippetLatexGroups[0].entries()].map(
                ([key, { part, text: partText }]) => (
                    <Text
                        ...
                        style={[...styles, partConfig[part].style]}>
                        {partText}
                    </Text>
                ),
            )}
            ...
            {[...textSnippetLatexGroups.entries()].map(([key, group]) =>
                group.hasOwnProperty('latex') ? (
                    ...
                    <Text
                        ...
                        style={[...styles, partConfig.heading.style]}>
                        {group.text}
                    </Text>
                ) : (
                    Platform.OS === 'ios' ? (
                        <Text key={key}
                        style={{ width: Dimensions.get('window').width * 0.6, color: 'red' }}>
                            {[...group.entries()].map(
                                ([key, { part, text: partText }]) => (
                                    <Text
                                        key={key}
                        ...
                        <View>
                            <Text key={key}
                                style={{ width: Dimensions.get('window').width * 0.7, color: 'red' }}>
                                {[...group.entries()].map(
                                    ([key, { part, text: partText }]) => (
                                        <Text
                                            key={key}
...
export default MarkDownParseText;
