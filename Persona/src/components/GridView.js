...
const GridView = memo(props => {
    ...
    const prepare = useCallback(() => {
        ...
        if (Math.abs(diff) == 1) {
            ...
            findIndex(
                self.items,
                (v, i) => keyExtractor(v.item) != keyExtractor(data[i]),
            ) >= 0
        ) {
            onUpdateData();
        }
        ...
        diff => {
            ...
            const animations = (diff < 0 ? items0 : items).reduce(
                (prev, curr, i) => {
                    ...
                    if (selectedItem && curr.item == selectedItem.item) {
                        return prev;
                    }
    ...
    const reorder = useCallback(
        (x, y) => {
            ...
            const itemIndex = findIndex(
                items,
                v => v.item == selectedItem.item,
            );

            if (isLocked || itemIndex == index) {
                return;
            }
