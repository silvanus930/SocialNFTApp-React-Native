...
export const isSameDayFromSeconds = (secondsOne, secondsTwo) => {
    let myDate, prevDate;

    if (secondsOne) {
        myDate = DateTime.fromSeconds(secondsOne);
    }

    if (secondsTwo) {
        prevDate = DateTime.fromSeconds(secondsTwo);
    }

    return (
        myDate.month === prevDate?.month &&
        myDate.day === prevDate?.day &&
        myDate.year === prevDate?.year
    );
};
